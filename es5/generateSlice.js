...
module.exports = function generateSlice(label, d1, d2, alpha, delta, color) {
  ...
  var path = ['M', p1.x, p1.y, 'A', d1, d1, 0, sweep, CLOCKWISE, p2.x, p2.y, 'L', p3.x, p3.y, 'A', d2, d2, 0, sweep, ANTICLOCKWISE, p4.x, p4.y, 'z'].join(' ');
  ...
  var snip = React.createElement(
    'g',
    null,
    React.createElement(
      'path',
      { d: path, fill: color || "rgba(0,0,0," + random() + ")", onMouseOver: mouseover, onMouseOut: mouseout },
      React.createElement(
        'text',
        { transform: transform, fontFamily: "verdana", textAnchor: "middle", fontSize: 40 },
        textContent
      )
    )
  );
